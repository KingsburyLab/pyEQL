...
def test_equilibrate_2L():
    ...
    assert np.isclose(solution.get_total_amount("Cu", "umol").magnitude, 1.9999, atol=1e-2)  # PHREEQCUI - 2e-06
...
def test_equilibrate_with_atm():
    ...
    assert np.isclose(s1.get_amount("CO2(aq)", "mol/L").magnitude, 1.42944e-05, atol=1e-2)  # PHREQCUI - 1.429e-05
