...
Each module within pyEQL has its own logger, with a NullHandler attached to it.
When a Solution is created, the user has an option to set a logging level (ERROR by
default). If set, a StreamHandler is added to the logger so that messages will go to
standard output.
...
CRITICAL    -   Used only in one instance, when a Solution is depleted of all solvent.
